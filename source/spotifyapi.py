...
def main():
	...
	if args.token:
		...
		spotify = SpotifyAPI.authorize(client_id='c485a6f9ec3143b79f4ecea88d4b7041', scope='playlist-read-private')
		#5c098bcc800e45d49e476265bc9b6934
	# Get the ID of the logged in user.
	me = spotify.get('me')
	subprocess.Popen('echo Logged in as {id}'.format(**me), shell=True)
	...
	for playlist in playlists:
		subprocess.Popen('echo Loading playlist: {name} ({tracks[total]} songs)'.format(**playlist), shell=True)
	...
	#with open(args.file, 'w', encoding='utf-8') as f:
		# JSON file.
	if args.format == 'json':
		json.dump(playlists, songs)
		
		# Tab-separated file.
	elif args.format == 'txt':
		for playlist in playlists:
			temp = []
			temp.append(playlist['name'])
			for track in playlist['tracks']:
				temp.append('{name} - {artists}'.format(
